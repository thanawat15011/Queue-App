...
const useQueuehooks = () => {
  ...
  useEffect(() => {
    // const socket = io.connect(`${GROBAL.BASE_SERVER.IO}`);
    fecthdata();
    const insertQueueData = (data) => {
        fecthdata();
    };
    socket.on('insertQueueData', insertQueueData);
    return () => {
      socket.off('insertQueueData', insertQueueData);
    };
    // num_quest_model
    //   .getNumQuestBy({ company_table_uuid: company_table_uuid })
    //   .then((response) => {
    //     const num =
    //       response.data.length > 0 ? response.data[0].num_quest + 1 : 1;
    //     setMaxQueue(num);
    //   });
    // return () => {
    //   socket.disconnect();
  ...
  useEffect(() => {
    // Calculate data.queue based on fetched data
    setData((prevData) => ({
      ...prevData,
  ...
  const checkSubmit = () => {
    if (data.name_service == "" || data.name_service == undefined) {
      ...
      return false;
    } else if (data.quest == null || data.quest == undefined) {
      Swal.fire({
        title: "กรุณาระบุคิว !",
        icon: "warning",
      });
...
const checkQueue = () => {
  ...
  for (let i = 0; i < quest.length; i++) {
    const row = quest[i];
    if (row.quest == data.quest) {
      Swal.fire({
        title: "คิวนี้มีอยู่แล้ว !",
        icon: "warning",
      });
      matchFound = false; 
      break; 
  ...
  const confirmCreate = async () => {
    if (checkSubmit()  && checkQueue() ) {
      ...
      num_quest_model
        .insertNumQuest({ num_quest: maxQueue })
        .then((response) => {
          if (response) {
            Swal.fire({
              title: "สำเร็จ",
  ...
  const clear_data = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        quest_trans_model
          .deleteAllData({ company_table_uuid: company_table_uuid })
          .then((response) => {
            if (response) {
              Swal.fire({
