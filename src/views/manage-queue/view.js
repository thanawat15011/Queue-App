              ...
              <Card sx={{ minWidth: 275 }}>
                ...
                >
                  ...
                  <FormControl sx={{ m: 1, minWidth: 220 }} size="small">
                    ...
                    <Select labelId="demo-select-small-label" id="demo-select-small" label="Age" onChange={setvalue} name="room" value={data.room}>
                      <MenuItem value="">
                        <em>None</em>
                      </MenuItem>
                      {examRoom.map((val, key) => {
                        return (
                          <MenuItem key={val.exam_room_table_uuid} value={val.exam_room_table_uuid}>
                            {val.exam_room_name}
                          </MenuItem>
                        )
                      })}
                ...
                >
                  <Button variant="contained" disableElevation onClick={confirmCreate}>
                    เพิ่มคิว
                  </Button>
                  <Button variant="contained" color="error" disableElevation onClick={clear_data}>
                    ล้างข้อมูล
                  </Button>
                </CardActions>
              </Card>
              ...
              <Paper sx={{ width: "100%", overflow: "hidden" }}>
                <TableContainer sx={{  maxHeight: "500px", overflowY: "auto"}}>
                  <Table stickyHeader aria-label="sticky table">
                    <TableHead>
                      <TableRow>
                        <TableCell style={cellStyle()}>ชื่อผู้รับบริการ</TableCell>
                        <TableCell style={cellStyle()}>ห้องตรวจ</TableCell>
                    ...
                    <TableBody>
                      {quest.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
                        <TableRow hover role="checkbox" tabIndex={-1} key={row.quest_table_uuid}>
                          ...
                          <TableCell style={cell()}>{row.quest}</TableCell>
                          <TableCell style={cell()}>{row.exam_room_name != null && row.quest != null ? row.status == 1? 'เรียกพบ': row.status == 2?'เรียกพบแล้ว' : 'รอ' : "รอกรอกข้อมูล"}</TableCell>
                          <TableCell>
                            {/* <IconButton color="primary" onClick={playAudio}> */}
                            {
                              row.status == 2 || (row.exam_room_name == null && row.quest == null)? (  
                              <IconButton
                                onClick={() =>
